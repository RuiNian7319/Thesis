...
\subsubsection{Cost Function and Performance Assessment}

The mean squared error cost function is used and is given by:
\begin{equation}
    Loss = \frac{1}{n}\sum\limits^n_{i=1}(\hat{y}_i - y_i)^2
    \label{eq:08MSE}
\end{equation}
where $n$ is the number of samples in the current optimization step.  $\hat{y}_i$ and $y_i$ are the $i^{th}$ predicted and actual labels, respectively. 
