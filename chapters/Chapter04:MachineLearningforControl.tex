...
Figure \ref{fig:04CSTR_1} shows the new state and input trajectories when the sampling time reduces to 1.  In this system, the dynamics are too slow and do not finish in 1 second.  Hence, RL showed slight sub-optimality due to the system not being Markovian.  In fact, deep RL showed an offset from the optimal set point.  This extremely small off-set is difficult to identify for the agent when using deep learning function approximation; however, using the reward proposed in Equation \ref{eq:04rl_cost}, this was eliminated by putting more emphasis on the off-set.
\begin{figure}[H]
    \centering
