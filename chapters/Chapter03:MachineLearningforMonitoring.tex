...
This project aims to firstly reduce the amount of alarm floods to prevent overwhelming the operators. Secondly, an alarm prioritization algorithm will be introduced so operators can focus their attention on the highest impact alarms first.
...
The WWTP contains 145 states, 2 control actions, and 14 disturbances.  The input states are the characteristics (flow rate, composition, etc.) of the influent into the WWTP.  There are three sets of inputs available to the WWTP: i) Dry weather data. ii) Rain weather data. iii) Storm weather data.  The dry weather data was used for the results presented in this section. For more information, please refer to the Benchmark Simulation No.1 documentation \cite{wwtp}.

There exists five environmental constraints on the process: 1) Total nitrogen ($N_{tot}$); 2) chemical oxygen demand (COD); 3) $NH_4^+ + NH_3$ nitrogen ($S_{NH}$); 4) total suspended solids (TSS); 5) biochemical oxygen demand (BOD). The threshold for the environmental constraints is given as follows:
\[
    Alarm = 
\begin{cases}
    On,              & N_{tot}  > 10 \\
...
Alarm flooding is one of the biggest problems in alarm management.  During abnormal events in the plant, thousands of alarms may go off simultaneously due to poor alarm system design, rendering the whole system useless.  The first algorithm in the SMART alarm management system is alarm reduction.  The general idea of the alarm reduction algorithm is to create a sequence dictionary.  The dictionary comprises of sequences of alarms that often occur together.  For example, if alarms 1 and 2 occur frequently together, an alarm sequence, \emph{Sequence 1}, could be generated representing alarms 1 and 2.  With this, the number of alarms would be halved.  More significant reduction in alarms can be achieved if 10 or 100 alarms often occur together.  The steps for the alarm reduction algorithm are shown in Table \ref{alg:03AlarmReduce}.
...
\begin{table}[H]
	...
	\hspace{0.5cm} Group alarms in pairs \\ 
	\hspace{1cm} \textbf{Save} grouped alarms in list \\
	\hspace{0.5cm} Group alarms in triplets \\
	\hspace{1cm} \textbf{Save} grouped alarms in list \\
	\hspace{0.5cm} ... \\
	\hspace{0.5cm} \textit{Repeat until all alarms are one group} \\
	\textbf{Sequence Identifier} \\ 
	\hspace{0.5cm} Generate all alarm sequences occurring in plant \\
	\hspace{0.5cm} Count amount of times each \textbf{grouped} alarm occurred \\
	...
	\hspace{0.5cm} Alarms from plant in 0, 1, 2 \\
	\hspace{0.5cm} Alarms converted to words \\
	...
	\hspace{1.5cm} ... \\
	\hspace{1.5cm} ... \\
	\hspace{1cm} \emph{Repeat until historical alarms is one group} \\
	\hline
	\end{tabular}}
...
\subsection{Alarm Reduction through Suppression}
