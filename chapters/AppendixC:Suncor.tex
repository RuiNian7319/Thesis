...
\begin{figure}[h]
    \centering
    \includegraphics[scale=0.4]{images/suncor/08RM06A.png}
...
The new communication framework for Line B is shown in Figure \ref{fig:08scheduleV2}. Here, the desired flow rate is sent to the operator and optimization tool.  The tool will then identify the most effective set points to achieve the desired set point and recommend them to the operators.  The tool is fully capable of automating the pipeline in open loop; however, initial performance skepticism must be mediated before doing so.
...
\begin{figure}
     ...
     \begin{subfigure}[b]{0.9\textwidth}
         ...
         \caption{CIG heavy DRA sensor reading.}
         \label{fig:08CIGheavy}
     \end{subfigure}
     \begin{subfigure}[b]{0.9\textwidth}
         \centering
         \includegraphics[width=\textwidth]{images/suncor/08CIGSweet.eps}
         \caption{CIG light DRA sensor reading.}
         \label{fig:08CIGlight}
     \end{subfigure}
     \begin{subfigure}[b]{0.9\textwidth}
         \centering
         \includegraphics[width=\textwidth]{images/suncor/08AultSour.eps}
         \caption{Ault heavy DRA sensor reading.}
         \label{fig:08Aultheavy}
     \end{subfigure}
     \begin{subfigure}[b]{0.9\textwidth}
         \centering
         \includegraphics[width=\textwidth]{images/suncor/08AultSweet.eps}
         \caption{CIG light DRA sensor reading.}
         \label{fig:08Aultlight}
     \end{subfigure}
...
\begin{enumerate}
    ...
    \item Remove data when only light or heavy crude was sent through the pipeline.
\end{enumerate}
...
In normal operations, the density fluctuates between 10 - 50 API, depending on the crude present in the pipeline.  After analysis, the Fort Lupton densitometer was behaving abnormally compared to other densitometer and is shown in Figure \ref{fig:08Density}.  After confirming with Suncor that the densitometer was behaving abnormally, the variable was removed.  
\begin{figure}[h]
     \centering
...
\begin{table}[h]
    ...
    \begin{tabular}{ p{6cm} | c }
        ...
        CIG \& Ault light DRA (ppm)             &      20 - 40            \\
        CIG \& Ault heavy DRA (ppm)              &      20 - 40            \\
    \end{tabular}}
...
\begin{table}[h]
    ...
    \begin{tabular}{ c | c }
        ...
        2$\times$ On/off Ault Pump         &  2$\times$ light \& 2$\times$ heavy DRA  \\
...
\begin{table}[h]
    ...
    \begin{tabular}{c|c}
         ...
         $API^{CIG} \geq 30: DRA_{light}^{CIG}$ = On & $P_{Outlet}^{Ault} \leq 1613 \text{ psi}$ \\
         
         $API^{CIG} \leq 30: DRA_{heavy}^{CIG}$ = On  & $P_{Outlet}^{FL} \leq 1613 \text{ psi}$  \\
         
         $API^{Ault} \geq 30: DRA_{light}^{Ault}$ = On & $20 \text{ ppm} \leq DRA \leq 40 \text{ ppm}$                 \\
         
         $API^{Ault} \leq 30: DRA_{heavy}^{Ault}$  = On  & $105 \text{ Amps} \leq VFD^{Chey} \leq 322 \text{ Amps}$   \\
