...
Similarily, Equation \ref{eq:04r3} considers the pumping energy cost.
\begin{equation}
    r_3 =
\begin{cases}
...
\begin{table}[H]
	...
	\hspace{2.25cm} \textbf{Observe} x_{O_2, t} \\
	\hspace{2.25cm} \textbf{If} U(0, 1) > $\epsilon$ \\
	\hspace{3cm} \textbf{Do} U(0, 1) > $\epsilon$ \\
...
\hspace{2cm} $N_t(x, u) \leftarrow N_t(x, u) + 1$ \\
\hspace{2cm} $T(x, \neq u) \leftarrow t(x, \neq u) + 1$ \\
\hspace{1cm} until convergence or x is a terminal state \\
...
\begin{table}[H]
	...
	\textbf{Initiate Plant Operations} \\
	\hspace{0.75cm} \textbf{Regulatory Control Layer} \\
	\hspace{0.75cm} Observe nitrogen level, $x_{N_2}$, in unit 5\\
