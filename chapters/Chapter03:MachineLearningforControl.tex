...
Once activated, the control agent provides recommendations to stabilize the process. A $Q$-learning active FTC is used for this study because of its adaptive nature and ability to acknowledge future rewards \cite{rl_control_ref10}.
...
During training, exploration of the environment is mandatory to avoid locally optimal policies. This study uses UCB action selection to explore actions based on its potential of being optimal \cite{sutton}:
\begin{equation}
...
The Wood-Berry distillation tower was used to validate the feasibility of the proposed FTCS. This tower contains one feed, two inputs and two outputs.  The feed stream containing methanol and water is characterized by the inlet mass composition $Z_f$. The control inputs are the reflux and steam flow rates, $R \; (lb/min)$ and $S \; (lb/min)$. Furthermore, the outputs are characterized by the methanol mass fraction, $X_D$ and $X_B$. Objectively, the distillation column aims to completely separate methanol and water.
\begin{figure}[h]
    \begin{center}
...
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=9.5cm]{images/ftc/Case3_Plot.eps}
        \caption{\label{fig:case_study3} Trade-off between conflicting objectives (Case 3).}
    \end{center}
\end{figure}

Case 4 was used to explore adaptability of the FTCS. A pre-trained control agent for regulating $X_D$ to 100 was re-tasked to regulate $X_B$ to 0.  In Fig. \ref{fig:case_study4}, a fault occurred at $t = 300$ in the  


Here, the control agent was able to completely adapt to the new operating objective by solely experiencing the new reward function. Adaptation speed can also be controlled by tuning learning rate $\alpha$.  

\begin{figure}[h]
    \begin{center}
        \includegraphics[width=9.5cm]{images/ftc/Case4_Plot.eps}
...
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=9.5cm]{images/ftc/time_to_mediate.pdf}
        \caption{\label{fig:time_to_mediate} Time required to mediate faults of different magnitudes averaged over 1000 simulations.  Shaded regions correspond to one standard deviation.}
    \end{center}
\end{figure}
...
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=9.5cm]{images/ftc/training_time.pdf}
        \caption{Fault mediation time and error accumulation vs. \# of training steps averaged over 30 runs. Shaded regions correspond to one standard deviation.}
        \label{fig:training_time} 
    \end{center}
\end{figure}

\subsection{Stability, Convergence and Comp. Complexity}
The stability of RL is guaranteed assuming a Lipschitz continuous model and confining exploration to within the region of attraction, given a bounded input \cite{stability_rl_ref21}.  In this study, the system is globally asymptotically stable with the region of attraction spanning the entire state space and is therefore stable. For convergence, given learning rates $0 \leq \alpha_n < 1$, bounded rewards $|r_n| \leq \mathcal{R}$ and:
\begin{equation}
    \sum\limits^{\infty}_{i = 1}\alpha_i(x, u) = \infty, \; \;  \sum\limits^{\infty}_{i = 1}\alpha_i^2(x, u) < \infty, \forall x, u,
\end{equation}
the tabular $Q$-learning values, $Q_n(x, u) \rightarrow Q^*(x, u)$ as $n \rightarrow \infty, \forall x, u$ with probability 1 --- the optimal result given such stochastic conditions \cite{convergence_rl_ref20}.  Assuming \textit{tabula rasa}, the computational complexity to reach the goal state for the first time during training is $O(p^3)$ \cite{complexity_rl_ref19}. Online evaluation complexity is approximately $O(mlog(p) + qlog(q))$ compared to $O(N(p^2 + q^2))$ for traditional optimal control solvers, where $N$ is the control horizon. Hence, RL evaluates much faster online compared to traditional optimal control methods.
