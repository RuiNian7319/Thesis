...
Finally, the data set is then manually pre-processed to remove data from badly behaving sensors and irregular operating conditions. Figures \ref{fig:08Density} to \ref{fig:08DRA} show some examples of poor data.  
...
Furthermore, both sweet and sour crude are sent into the pipeline in a cyclical fashion due to the hydraulic dynamics of the pipeline. The sour crude is too heavy to be transported for sustainable periods otherwise. However, downstream demand for each product can disrupt this operating cycle.  From Figure \ref{fig:08API}, it can be seen that there were extended periods of time where only sweet or sour crude were being transported.  Such scenarios deviate from normal operations and were removed from the data.
...
DRA parts per million (ppm) is measured continuously for the control loop of the DRA injection pumps. 
...
Finally, because the pipeline is fully loaded and crude is incompressible, pressure changes upstream will be propagated down the pipeline at close to the speed of sound \cite{fluid_mechanics}.  


A comprehensive list of the manual data pre-processing procedures is as follows:
\begin{enumerate}
    \item Shift flow rates up one time step: $Q_{t + 1} = Q_t$.
    ...
    \item Remove data when DRA was not being used.
\end{enumerate}
