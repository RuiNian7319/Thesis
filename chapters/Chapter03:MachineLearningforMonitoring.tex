...
\begin{table}[H]
	...
	\hspace{1cm} \textbf{Group} previous thress alarms \\
	\hspace{1.5cm} Check sequence dictionary for match \\
	\hspace{1.5cm} \textbf{If match} \\
...
Tables 7 and 8 shows the comparsion of a traditional alarm log compared to the proposed algorithm.  Comparing the alarm logs, it can be seen that the SMART alarm system was able to vastly reduce the amount of alarms, while putting the most important alarms on the top of the list.
...
The alarm prioritization algorithm is given in Table 9.  The value-sequence dictionary is generated live and need to be saved so if the same sequence occurs in the future, a value can be assigned to the sequence for prioritization purposes.
