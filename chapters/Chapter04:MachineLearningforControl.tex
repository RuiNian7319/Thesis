...
\end{figure}
The states and actions, $x$ and $u$, correspond to $\varepsilon$ and $\Delta u$, respectively.  The discount factor, $\gamma$, of the agent was 0.9. Overall, the agent was trained for 2,000,000 update steps corresponding to 115.7 days of continuous operating experience. After every 400$^{th}$ update step, the agent is reset back to the initial states given in Equations \ref{eq:initial_p} and \ref{eq:initial_u} to prevent controller saturation during initial episodes. 

Exploration-wise, the agent starts with a equiprobable random policy ($\epsilon$ = 1), which decays linearly until $\epsilon = 0.1$ by the 500,000$^{th}$ update. Likewise, the learning rate, $\alpha$, is initiated at 0.7 and decays linearly until 0.001.
