...
The uniform sampling incremental supervised learning (USIS) algorithm is proposed to overcome this issue and to enhance data efficiency for adaptive ML techniques.  USIS is a combination of adaptive resonance theory, uniform sampling, experience replay (from reinforcement learning theory), where each update step is outlier free, data is efficiently used many times, and data is uniformly sampled across the distribution of the model to prevent catastrophic interference.
...
Uniform sampling is random sampling method where proportions of each desired group are forced to be sampled equally.  In the context of process control where data typically remain constant over long periods of time, uniform sampling can guarantee data variety during an incremental learning update step.

Lastly, experience replay is a method that gained popularity when it was first introduced to reinforcement learning to improve data effciency \cite{dqn}.

\subsubsection{USIS Algorithm}

Figure \ref{fig:02USIS} shows the architecture for the USIS algorithm.  

- Add effects of memory replay here for tuning of USIS.
...
Uniform sampling in USIS can also be swapped out for importance sampling, where high error examples are prioritized for the next sampling cycle (motivation from \cite{prioritize_exp_replay}.  Although, such a method may prioritize noisy, near-outlier data points and introduce model divergence.

as proposed in \cite{}

\subsection{Conclusion: Cheaper, More Accurate, and Adaptive?}
