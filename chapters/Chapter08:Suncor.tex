...
\begin{equation}
    ...
    \label{eq:08LS}
\end{equation}
where $x$ is a vector of features and superscript $T$ denotes the transpose operation.
...
The performance assessment of the quadratic and square root models are shown in Table \ref{tab:08quad_performance} and \ref{tab:08sq_performance}, respectively. Compared to constrained least squares, MAE and RMSE reduced by up to 10\% and 9\% when using the polynomial models, respectively.  Performance of the square root model was about 3.5\% better than the quadratic model.
\begin{table}[h]
    \centering
    {\setstretch{1.2}
    \begin{tabular}{ c | c | c | c}
...
The performance assessment of the two LPV models are shown in Tables \ref{tab:08cluster1_reg} and \ref{tab:08cluster2_reg}. Overall, the LPV models are able to further reduce modelling error. Model 1 was only used to predict high flow rate scenarios. Nevertheless, its MAE and RMSE were almost identical to the constrained least squares model used to predict for all data.  Moreover, RMSE and MAE for model 2 were 7\% lower compared to the constrained least squares. 
\begin{table}[h]
    \centering
    {\setstretch{1.2}
    \begin{tabular}{ c | c | c | c}
