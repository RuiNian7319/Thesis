...
The second alarm reduction method suppresses low impact alarms altogether.  First, the algorithm uses RL to assign credits to different alarms via the optimal value or action-value functions, $v^*(x)$ and $Q^*(x, u)$, respectively.  During alarming events, alarms failing to exceed a threshold value will be suppressed.  Intuitively, this algorithm assigns different values to 


Both Markov reward processes (MRP) or Markov decision processes (MDP) can be used to represent this system.  MRPs are used if the system already contains existing controls and only the alarm suppression algorithm is of interest.  MDPs should be used if the plant managers also want to understand the optimal actions during different alarm events.


Because $Q$-values may have significantly different magnitudes across different processes, the $Q$-values are first normalized using:
\begin{equation}
