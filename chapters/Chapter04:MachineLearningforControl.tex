...
\section{Continuous Control using Deep RL}
...
$$x_{ss} = 5.0$$ 
$$u_{ss} = 10.0$$
Here, MPC used a prediction and control horizon of 100, with $Q_{mpc}$ and $R_{mpc}$ as 0.1 and 0.1, respectively.  The RL hyper parameters are given in Tables \ref{tab:04tab_rl} and \ref{tab:04deep_rl}

\begin{table}[H]
\caption{Tabular RL hyper parameters.}
\label{tab:04tab_rl}
\centering
\begin{tabular}{c|c}
\textbf{Hyper Parameter}     & \textbf{Value}  \\
\hline
States, $x$	             	& $\varepsilon = [0, 0.18, ..., 8]_{1 \times 45} $		 \\
Actions, $u$               & $\Delta u = [5, 5.22, ..., 15]_{1 \times 45}$		\\
...
Figure \ref{fig:04SISOcomp} shows the input and state trajectories of the four control algorithms.  
